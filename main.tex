...
Although the AGC minimization problem is proved to be NP-hard, we can utilize the monotonicity and supermodularity of AGC to develop a fast greedy algorithm called \text{ApproxAGCM}.
\text{ApproxAGCM} is based on \(\text{Approx}\mathcal{HK}\) and yields an approximation factor of \(1-\frac{k}{k-1}\cdot\frac{1}{e}-\epsilon\) for any error parameter \(\epsilon\in(0,1)\).
...
\begin{itemize}
    ...
    \item Due to the monotonicity and supermodularity of AGC, we develop an efficient greedy algorithm called \text{ApproxAGCM} based on \(\text{Approx}\mathcal{HK}\) to approximately solve the AGC minimization problem.
    \item Extensive experiment results demonstrate that both of \(\text{Approx}\mathcal{HK}\) and \text{ApproxAGCM} are accurate and scalable, which can be applied to networks with millions of vertices.
\end{itemize}
...
For the convenience of representing specific elements in vectors and matrices, we use \(a_{i}\) to represent the \(\myord{i}\) element of vector \(\veca\), and \(\mata_{[i,j]}\) to represent the entry at position \((i,j)\) in matrix \(\mata\).
...
For example, \(\mata_{[S,j]}\) represents the subvector that includes matrix elements with row indices in \(S\) and column index as \(j\).
Similarly, \(\veca_{-S}\) represents the subvector of \(\veca\) obtained by removing elements with indices in set \(S\), \(\mata_{-S}\) represents the submatrix of \(\mata\) obtained by removing elements with row indices or column indices in set \(S\).
...
Mathematically, the topological and weighted properties of a graph \(\gr\) are encoded in its generalized adjacency matrix \(\mata\) with the entry \(a_{ij}\) denoting the adjacency relation between vertices \(i\) and \(j\). If vertices \(i\) and \(j\) are linked to each other by an edge, then we denote this edge as \(e=\edge{u}{v}\), with \(a_{ij}= a_{ji}=w_{e}> 0\). Otherwise, \(a_{ij}=a_{ji}=0\) indicating that vertices \(i\) and \(j\) are not adjacent. In a weighted graph \(\gr\), the degree \(d_i\) of a vertex \(i\) is defined by \(d_i=\sum_{j=1}^n a_{ij}\)~\cite{BaBaPaVe04}, where we denote the maximal vertex degree as \(d_{\max}=\max\setof{d_i|i\in V}\).
The diagonal degree vector of graph \(\gr\) is defined to be \(\vecd=\mypar{d_1,d_2,\cdots,d_n}^\top\), while the diagonal degree matrix of graph \(\gr\) is defined to be \({\matd} = {\rm diag}(d_1, d_2, \ldots, d_n)\), and the Laplacian matrix of \(\gr\) is \({\lap}={\matd}-{\mata}\).
...
Similarly, We can also define the resistance distance between a vertex and a group of vertices.
In the electrical network \(\bar{\gr}\) corresponding to graph \(\gr\), we can ground all the vertices in group \(S\) to set their voltage as zero.
Then the resistance distance \(R_{iS}\) between vertex \(i\) and vertex group \(S\) is defined as the voltage of \(i\) when a unit current enters \(\bar{\gr}\) and leaves it at any vertex in \(S\).

Based on the physical explanation of effective resistance, the resistance distance \(R_{iS}\) is smaller than the effective resistance of any simple path connecting vertex \(i\) with any vertex \(j\in S\).
To clarify, if we denote the simple path connecting vertex \(i\) and an arbitary vertex in \(S\) as \(\pscr_{iS}\), then we have:
...
For a connected weighted graph \(\gr\) with \(n\) vertices, the classical random walk model of \(\gr\) can be described by the transition matrix \(\matp\in\rea^{n\times n}\): at any time step, the walker located at vertex \(i\) moves to vertex \(j\) with probability \({\matp_{[i,j]}=d_i^{-1}\mata_{[i,j]}}\).
...
\begin{IEEEproof}
    ...
    \begin{equation}\label{EE05}
        \begin{split}
            ...
            &=\frac{1}{s} \sum_{i=1}^{n} d_i \sum_{z=1}^{n} d_z \left({\lap}_{[j,j]}^{\dagger}-{\lap}_{[i,j]}^{\dagger}-{\lap}_{[j,z]}^{\dagger}+{\lap}_{[i,z]}^{\dagger}\right) \\
        \end{split}
    \end{equation}
...
\section{Absorbing Group Centrality (AGC)}
...
In this section, we provide the definition of Absorbing Group Centrality (AGC).
We also discuss the connections between AGC and other quantities related to random walk, such as the random detour time, the Kemeny constant and the equivalent electrical network of the augmented graph.
Furthermore, we demonstrate monotonicity and supermodularity of AGC \(\manc{\cdot}\).
...
According to \eqref{eq:AGC} and \lemref{lemma:ST}, we can easily compute AGC of any given vertex group \(S\) by calling \(\mathtt{Solve}\mypar{\lap_{-S},\vecd_{-S},\delta}\).
The detail of this approximation algorithm is skipped as it simply utilizes \lemref{lemma:ST}.
...
By leveraging this connection along with \thmref{thm:connection-multiple}, we can establish another link between AGC and the equivalent electrical network of the augmented graph.
...
\figref{fig:augmented-graph} displays an example of creating the augmented graph \(\gr'\) from the original graph \(\gr\).
...
\begin{theorem}[Monotonicity of AGC]\label{thm:mono}
...
\begin{IEEEproof}
    According to \defref{def:manc}, when a random walker arrives at vertices within \(S\), it must have arrived at vertices within \(S+u\). Therefore \(H_{vS}\ge H_{v\mypar{S+u}}\) holds for any vertex \(v\in V\), which concludes our proof.
\end{IEEEproof}

\begin{theorem}[Supermodularity of AGC]\label{thm:supermod}
...
\begin{IEEEproof}
    ...
    As is mentioned in proof of \thmref{thm:mono}, \(T_{vS}\ge T_{vT}\) holds for any vertex \(v\in V\).
...
In \lemref{HjK}, we have shown that \(H_u=d\mypar{\vece_u-\vecpi}^\top\lap^{\dagger}\mypar{\vece_u-\vecpi}\).
...
As a result, we can optimize the simple greedy algorithm outlined in~\algoref{algo:exact-agcm}, and reduce the time complexity from \(O(kn^4)\) to \(O(kn^3)\).

\begin{algorithm}
    \caption{\text{ExactAGCM}\((\gr,k)\)}
...
Afterwards, we prove that \algoref{algo:exact-agcm} has a \(1-\frac{k}{k-1}\cdot\frac{1}{e}\) approximation factor.
\begin{theorem}
...
Despite the time complexity optimization proposed in \algoref{algo:exact-agcm}, this simple greedy algorithm for solving AGCM still requires matrix inversion, making it unsuitable for large networks.
...
\subsection{Details of Algorithms \text{ApproxDelta} and \text{ApproxAGCM}}

By combining Lemmas~\ref{lem:approx-numer} and~\ref{lem:approx-denom}, we propose the \text{ApproxDelta} algorithm for approximating \(\Delta(u,S)\) when \(S\neq\varnothing\).
The \text{ApproxDelta} algorithm calls algorithm \(\mathtt{Solve}\) in \lemref{lem:approx-denom} for \(q=\ceil{24(\epsilon/7)^{-2}\log n}\) times, resulting in a complexity of \(\tilde{O}\mypar{m\epsilon^{-2}}\).
Here, \(\tilde{O}(\cdot)\) hides the poly(\(\log n\)) factors.
Additionally, we can provide an approximation guarantee for this algorithm.

\begin{algorithm}
    \caption{\text{ApproxDelta}\((\gr,S,\epsilon)\)}
    ...
    \(\vecx'\gets\mathtt{Solve}(\lap_{-S},\vecd_{-S},\delta_1)\)\;
    \(\matq\gets\text{GenerateRandomMatrix}(q,m)\)\;
    \(\matr\gets\text{GenerateRandomMatrix}(r,n)\)\;
    ...
    \ForEach{\(u\in V\setminus S\)}{
        \(\Delta'(u,S)\gets\frac{\vecx'^2}{d\mypar{\Abs{\matx'\vece_i}^2+\Abs{\maty'\vece_i}^2}}\)\;
    }
...
By combining \algoref{ALG01} and \algoref{algo:approxdelta}, we are able to create a faster greedy algorithm called \text{ApproxAGCM} with a \(1-\frac{k}{k-1}\cdot\frac{1}{e}-\epsilon\) approximation factor.
The complexity of \text{ApproxAGCM} is \(\tilde{O}\mypar{km\epsilon^{-2}}\), which is nearly linear, as it calls \(\text{Approx\(\mathcal{HK}\)}\) once and \text{ApproxDelta} \(k-1\) times.

\begin{algorithm}
    \caption{\text{ApproxAGCM}\((\gr,k,\epsilon)\)}
    \label{algo:approx}
    \Input{
        A connected weighted undirected graph \(\gr=(V,E,w)\);
    ...
    \For{\(i=2,3,\dots,k\)}{
        \(\setof{\Delta'(u,S)|u\in V\setminus S}\gets\text{ApproxDelta}(\gr,S,\epsilon)\)\;
        \(u^*\gets\argmax_{u\in V\setminus S}\setof{\Delta'(u,S)}\)\;
        \(S_i\gets S_{i-1}\cup\setof{u^*}\)\;
    }
    \Return{\(S_k\)}
...
\begin{itemize}
    ...
    \item \text{ExactAGCM} and \text{ApproxAGCM} for solving the AGC Minimization problem.
\end{itemize}
...
\subsection{Results for \text{ExactAGCM} and \text{ApproxAGCM}}

We first compare the performance of \text{ExactAGCM} and \text{ApproxAGCM} with the optimum solution on four toy networks~\cite{Ku13}: \(\mathit{Zebra}\) with 23 vertices, \(\mathit{Zachary\ karate\ club}\) with 34 vertices, \(\mathit{Contiguous\ USA}\) with 49 vertices and \(\mathit{Les\ Miserables}\) with 77 vertices.
...
We then compare the smallest AGC value with the solution provided by \text{ExactAGCM} and \text{ApproxAGCM}, whose results are shown in \figref{pic:compare-effect-optimum}.
During the execution of \text{ApproxAGCM}, we set the error parameter \(\epsilon\) to \(0.2\).
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[scale=\scaleoptimum,baseline]
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{AGC \(\manc{S}\) of vertex group \(S\) computed by three different algorithms(\text{ExactAGCM},\text{ApproxAGCM} and \text{OptimumAGCM}) on four networks: Zebra (a), Zachary karate club (b), Contiguous USA (c) and Les Miserables (d).\label{pic:compare-effect-optimum}}
\end{figure}

We then compare the performance of \text{ExactAGCM} and \text{ApproxAGCM} with three other algorithms: \text{Top-Absorb}, \text{Top-Degree}, and \text{Top-PageRank}.
\text{Top-Absorb} simply selects \(k\) absorbing vertices with the lowest absorbing random-walk centrality, while \text{Top-Degree} and \text{Top-PageRank} select the vertices with the largest degrees and PageRank values, respectively.
We execute these four algorithms on eight medium-sized networks.
During the execution of \text{ApproxAGCM}, we set the error parameter \(\epsilon\) to \(0.2\).
The results are shown in \figref{pic:compare-effect1} and \figref{pic:compare-effect2}.
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[scale=\scaleexact,baseline]
        ...
        \end{semilogyaxis}
    \end{tikzpicture}
    \caption{AGC \(\manc{S}\) of vertex group \(S\) computed by four different algorithms(\text{ExactAGCM},\text{ApproxAGCM},\text{Top-Absorb} and \text{Top-Degree}) on four networks: CA-GrQc (a), ego-Facebook (b), Euroroads (c) and US power grid (d).\label{pic:compare-effect1}}
\end{figure}
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[scale=\scaleexact,baseline]
        ...
        \end{semilogyaxis}
    \end{tikzpicture}
    \caption{AGC \(\manc{S}\) of vertex group \(S\) computed by four different algorithms(\text{ExactAGCM},\text{ApproxAGCM},\text{Top-Absorb} and \text{Top-Degree}) on four networks: Hamster friends (a), Hamster full (b), Reactome (c) and Route views (d).\label{pic:compare-effect2}}
\end{figure}

Finally, we demonstrate that the \text{ApproxAGCM} algorithm is significantly more efficient than the \text{ExactAGCM} algorithm, particularly when applied to large networks.
We test both algorithms on a wider range of real networks.
For each network, we use \text{ExactAGCM} and \text{ApproxAGCM} separately to solve the AGC minimization problem, with \(k=10\) and \(\epsilon=0.2\).
The running times of both algorithms are shown in \tabref{tab:running-time}.

From \tabref{tab:running-time}, we can observe that the running time of \text{ApproxAGCM} is proportional to the number of edges in the network, leading to an increase in the speedup ratio of \text{ApproxAGCM} as the network size grows.
Furthermore, \tabref{tab:running-time} indicates that when processing networks marked with \(\ast\),
\text{ApproxAGCM} remains usable while \text{ExactAGCM} fails due to its high time complexity.
...
\begin{table}[htbp]
    ...
    \fontsize{8.0}{8.8}\selectfont
    \begin{threeparttable}
        \caption{The running time (seconds, \(s\)) of \text{ExactAGCM} and \text{ApproxAGCM} (denoted as \text{Exact} and \text{Approx} in the table to save space) with various \(\epsilon\) on several realistic networks}
        \label{tab:running-time}
        \begin{tabularx}{8.25cm}{c p{1cm} p{0.5cm} p{0.5cm} p{0.5cm}}
            \toprule[1pt]
            \multirow{2}{*}{Network}      &
            \multirow{2}{*}{\text{Exact}} &
            \multicolumn{3}{c}{\text{Approx} (\(s\)) for various \(\epsilon\)}\cr
            \cmidrule{3-5}
                                          & (\(s\)) & \(0.3\) & \(0.25\) & \(0.2\)\cr
            \midrule
