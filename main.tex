...
By combining \algoref{ALG01} and \algoref{algo:approxdelta}, we are able to create a faster greedy algorithm called \textsc{ApproxMinGWC} with a \(1-\frac{k}{k-1}\cdot\frac{1}{e}-\epsilon\) approximation factor.
The complexity of \textsc{ApproxMinGWC} is \(\tilde{O}\mypar{km\epsilon^{-2}}\), which is nearly linear, as it calls \textsc{ApproxHK} once and \textsc{ApproxDelta} \(k-1\) times.
...
\begin{theorem}
    The algorithm \(S_k=\textsc{ApproxMinGWC}(\gr,k,\epsilon)\) takes a connected weighted undirected graph \(\gr=(V,E,w)\) ,a positive integer \(k\) and an error parameter \(\epsilon\in(0,1)\), then returns a vertex group \(S_k\) with capacity \(k\). When \(k>1\), the vertex group \(S\) satisfies
    \begin{align*}
...
\begin{itemize}
    \item \textsc{ApproxHK} for computing the walk centrality \(H_u\) of  every vertex and the Kemeny constant \(K\);
    \item \textsc{DeterMinGWC} and \textsc{ApproxMinGWC} for solving the GWC Minimization problem.
\end{itemize}
...
All experiments are conducted on a machine with 16-core 2.55GHz AMD EPYC Milan CPU and with 64GB of RAM. We implement the approximation algorithms \textsc{ApproxHK} and \textsc{ApproxMinGWC} in \textit{Julia v1.8.5}, where the \(\mathtt{Solver}\) is from~\cite{GaKySp23}. %, the \textit{Julia language} implementation for which is available  on the website~\footnote{http://danspielman.github.io/Laplacians.jl/latest/}.

\subsection{Results for \textsc{ApproxHK}}

We begin by evaluating the effectiveness of algorithm \textsc{ApproxHK} in approximating centrality \(H_u\) across a diverse range of realistic networks from various domains. Our investigation focuses on examining the efficiency of \textsc{ApproxHK}.
...
\begin{table*}[htbp]
    ...
    \begin{threeparttable}
        \caption{The running time (seconds, \(s\)) of \textsc{ExactHK} and \textsc{ApproxHK} with various \(\epsilon\) on several realistic networks.}
...
\subsection{Results for \textsc{DeterMinGWC} and \textsc{ApproxMinGWC}}
...
We then compare the smallest GWC value with the solution provided by \textsc{DeterMinGWC} and \textsc{ApproxMinGWC}, whose results are shown in \figref{pic:compare-effect-optimum}.
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[scale=\scaleoptimum,baseline]
        \begin{axis}[
                ...
                legend entries={\textsc{ApproxMinGWC},\textsc{DeterMinGWC},\textsc{OptimumMinGWC}},
            ]
            \addplot+[
                mark=pentagon*,
                mark size = 3pt
    ...
    \begin{tikzpicture}[scale=\scaleoptimum,baseline]
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{GWC \(\manc{S}\) of vertex group \(S\) computed by three different algorithms(\textsc{DeterMinGWC},\textsc{ApproxMinGWC} and \textsc{OptimumMinGWC}) on four networks: Zebra (a), Zachary karate club (b), Contiguous USA (c) and Les Miserables (d).\label{pic:compare-effect-optimum}}
\end{figure}

We then compare the performance of \textsc{DeterMinGWC} and \textsc{ApproxMinGWC} with three other algorithms: \text{Top-Absorb}, \text{Top-Degree}, and \text{Top-PageRank}.
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[scale=\scaleexact,baseline]
        \begin{semilogyaxis}[
                ...
                legend entries={\textsc{ApproxMinGWC},\textsc{DeterMinGWC},Top-Absorb,Top-Degree,Top-PageRank},
                ymax=800,
            ]
            \addplot+[
                mark=pentagon*,
    ...
    \begin{tikzpicture}[scale=\scaleexact,baseline]
        ...
        \end{semilogyaxis}
    \end{tikzpicture}
    \caption{GWC \(\manc{S}\) of vertex group \(S\) computed by four different algorithms(\textsc{DeterMinGWC},\textsc{ApproxMinGWC},\text{Top-Absorb} and \text{Top-Degree}) on four networks: CA-GrQc (a), ego-Facebook (b), Euroroads (c) and US power grid (d).\label{pic:compare-effect1}}
\end{figure}
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[scale=\scaleexact,baseline]
        ...
        \end{semilogyaxis}
    \end{tikzpicture}
    \caption{GWC \(\manc{S}\) of vertex group \(S\) computed by four different algorithms(\textsc{DeterMinGWC},\textsc{ApproxMinGWC},\text{Top-Absorb} and \text{Top-Degree}) on four networks: Hamster friends (a), Hamster full (b), Reactome (c) and Route views (d).\label{pic:compare-effect2}}
\end{figure}
...
\begin{table}[htbp]
    ...
    \fontsize{8.0}{8.8}\selectfont
    \begin{threeparttable}
        \caption{The running time (seconds, \(s\)) of \textsc{DeterMinGWC} and \textsc{ApproxMinGWC} (denoted as \text{Exact} and \text{Approx} in the table to save space) with various \(\epsilon\) on several realistic networks}
