...
\begin{IEEEproof}
    ...
    Thus, we can simplify~\eqref{eq:MANC} as:
    \begin{equation*}
...
\begin{theorem}
    The algorithm \(S_k=\textsc{Exact}(\gr,k)\) takes a connected weighted undirected graph \(\gr=(V,E,w)\) and a positive integer \(k\), then returns a vertex group \(S_k\) with capacity \(k\).
...
We first approximate the numerator of \(\Delta(u,S)\), which requires the approximation of computing \(\vece_u^\top\lap_{-S}^{-1}\vecs_{-S}\).
Since \(\lap_{-S}\) is a SDDM matrix, we can use~\lemref{lemma:ST} to avoid computing matrix inverse \(\lap_{-S}^{-1}\).
...
\begin{IEEEproof}
    ...
    On the other hand, since \(\vecx_i=\vece_i^\top\lap_{-S}^{-1}\vecs_{-S}\) denotes the expected hitting time of a random walker starting from vertex \(i\), we have \(\vecx_i\ge1\).
...
Subsequently, we attempt to approximate the denominator of \(\Delta(u,S)\), which can be recast in an euclidian norm by using \lemref{lemma:JL}:
...
Although~\eqref{eq:denom-solver} indicates that we can avoid matrix inversion by using~\lemref{lemma:ST}, due to the dimension of the matrices \(\matw'\) and \(\matz\) being \(\abs{E}=m\) and \(\abs{V}=n\) respectively, the number of calls to \(\mathtt{Solve}\) is still unacceptable.
Therefore, we need to use~\lemref{lemma:JL} to reduce the dimension of the corresponding matrices.

From~\lemref{lemma:JL}, let \(\matq\in\rea^{q\times m},\matr\in\rea^{r\times n}\) denote the projection matrix constructed according to the definition of~\lemref{lemma:JL}, where \(q=r=\ceil{24\epsilon^{-2}\log n}\), then we have
...
Because \textsc{ApproxDelta} calls algorithm \(\mathtt{Solve}\) in \lemref{lem:approx-denom} for \(q=\ceil{24(\epsilon/7)^{-2}\log n}\) times, the complexity of \textsc{ApproxDelta} is \(\tilde{O}\mypar{m\epsilon^{-2}}\), where \(\tilde{O}(\cdot)\) hides the poly(\(\log n\)) factors.
...
\begin{algorithm}
    ...
    \Output{
        The margin \(\Delta(u,S)\) of MANC created by adding vertex \(u\) to \(S\) for any vertex \(u\in V\setminus S\)
    }
    Compute \(\lap\) and \(\vecs\)\;
    \(n\gets\abs{V},m\gets\abs{E}\)\;
    \(w_{\min}\gets\min\setof{w_e|e\in E},w_{\max}\gets\max\setof{w_e|e\in E}\)\;
    \(d\gets\vecs\vecone\),\(q,r\gets\ceil{24(\epsilon/7)^{-2}\log n}\)\;
...
Combining \algoref{ALG01} and \algoref{algo:approxdelta}, we can give the faster greedy algorithm \textsc{ApproxAGCM} with a \(1-\frac{k}{k-1}\cdot\frac{1}{e}-\epsilon\) approximate factor.
Since \textsc{ApproxAGCM} calls \(\text{Approx\(\mathcal{HK}\)}\) once and calls \textsc{ApproxDelta} \(k-1\) times, the complexity of \textsc{ApproxAGCM} is \(\tilde{O}\mypar{km\epsilon^{-2}}\), which is nearly linear.
...
\begin{algorithm}
    ...
    \Output{\(S_k\): A subset of \(V\) with \(\abs{S_k}=k\)}
    \(d\gets\vecs\vecone\),\(\vecpi\gets d^{-1}\vecs\)\;
