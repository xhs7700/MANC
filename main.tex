...
At each iteration of the naive greedy algorithm, one needs computing the maximum marginal gain $\Delta(u,S)$ for every candidate vertex in set $V \setminus S$. As shown in~\eqref{eq:GWC}, a straightforward calculation of  $\Delta(u,S)$ for every vertex \(u\) requires inverting a matrix, which needs $O((n-|S|)^3)$ time.  Thus, for the $(O(n-|S|))$ candidate vertices, the computation time at each iteration is $O((n-|S|)^4)$. Hence, the total computation complexity of the na\"{\i}ve greedy algorithm is \(O(kn^4)\) since $|S| \ll n$.
...
\lemref{lem:delta} demonstrate that for any empty set $S$ if \(\lap_{-S}\)  is already computed, by using~\eqref{eq:delta}  we can calculate \(\Delta(u,S)\) for any vertex \(u\in V\setminus S\) in $O((n-|S|)^2)$ time. This avoids directly inverting the matrix \(\lap_{-(S+u)}\), which needs time $O((n-|S|+1)^3)$. When \(S=\varnothing\),~\eqref{eq:delta} becomes invalid.  For this case, we use the result \(H_u=d\mypar{\vece_u-\vecpi}^\top\lap^{\dagger}\mypar{\vece_u-\vecpi}\) in~\lemref{HjK} to compute the walk centrality $H_u$ for every vertex $u \in V$, and add the vertex $u$ with the smallest $H_u$ to $S$.  Based on~\lemref{HjK} and~\lemref{lem:delta}, we propose a simple greedy algorithm \textsc {ExactMinGWC}$ (\gr,k)$ to solve the MinGWC problem, which is  outlined in~\algoref{algo:exact-gwcm}. The algorithm first computes $H_u$ for every vertex $u \in V$ in time $O(n^3)$. Then, it works in $k-1$ rounds. In each round, it evaluates  \(\Delta(u,S)\) for all the $ (n-|S|)$ vertices in set $V \setminus S$ (Line 6)  in $O((n-|S|)^3)$ time. Therefore, the whole running time of~\algoref{algo:exact-gwcm} is \(O(kn^3)\),  much lower than the time complexity   \(O(kn^4)\) of the  na\"{\i}ve greedy  algorithm.


\begin{algorithm}
    \caption{\(\text{ExactMinGWC}\)\((\gr,k)\)}
...
\begin{theorem}
    The algorithm $S_k$= \textsc {ExactMinGWC}$ (\gr,k)$ takes a connected weighted undirected graph \(\gr=(V,E,w)\) and a positive integer \(1 \geq k \ll n\), and returns a  group \(S_k\) of \(k\) vertices.
...
\subsection{Approximation of the Quantity $\mathbf{e}_u^\top \mathbf{L}_{-S}^{-1}\mathbf{e}_S$}

We first  approximate the quantity \(\vece_u^\top\lap_{-S}^{-1}\vecd_{-S}\). To this end, we introduce the following two lemmas.
...
\subsection{Approximation of the Quantity $\mathbf{e}_u^\top \mathbf{L}_{-S}^{-1}\mathbf{e}_u$}
