...
\begin{tikzpicture}[scale=0.25,baseline]
  \begin{axis}[
      xlabel={\(k\)},
      ylabel={\(M(S)\)},
...
\begin{tikzpicture}[scale=0.25,baseline]
  ...
  \end{axis}
\end{tikzpicture}
\captionof{figure}{MANC \(\manc{S}\) of node set \(S\) computed by three different algorithms(\textsc{Exact},\textsc{Approx} and \textsc{Optimum}).\label{pic:compare-effect-optimum}}
