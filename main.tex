...
It is evident that the straightforward algorithm presented above requires computing \(\Delta(u,S)\) for every vertex \(u\) in the graph during each iteration, resulting in \(O(n)\) matrix inverse operations due to~\eqref{eq:GWC}.
...
\begin{lemma}\label{lem:delta}
    Given a connected weighted undirected graph \(\gr=(V,E,w)\), the nonempty vertex group \(S\) and the Laplacian matrix \(\lap\), then for any vertex \(u\in V\setminus S\), we have
    \begin{equation}\label{eq:delta}
        \manc{S}-\manc{S+u}=\frac{\mypar{\vece_u^\top\lap_{-S}^{-1}\vecd_{-S}}^2}{d\mypar{\vece_u^\top\lap_{-S}^{-1}\vece_u}}.
    \end{equation}
\end{lemma}
...
However, during the first iteration of the greedy algorithm, when \(S=\varnothing\),~\eqref{eq:delta} becomes invalid due to the singularity of \(\lap\).
...
\lemref{HjK} and \lemref{lem:delta} demonstrate that we can calculate \(\Delta(u,S)\) for any vertex \(u\in V\setminus S\) by inverting the same matrix \(\lap_{-S}\) or by using the pseudoinverse of the same matrix \(\lap\).
...
As is shown in \lemref{lem:delta}, \(\Delta(u,S)\) is represented in the form of a fraction.
