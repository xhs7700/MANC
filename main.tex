...
\subsection{Physical Explanations of GWC}

\subsubsection{Connections with the random detour time}

After detailing GWC, we try to establish a connection between GWC \(\manc{S}\), the group random detour time \(D_{ij}(S)\) and the Kemeny constant \(K\).
...
In this paper, we explore the random detour time of multiple vertices, providing a physical explanation of GWC.
...
\begin{IEEEproof}
    ...
    \begin{equation}\label{eq:detour}
        ...
        \end{split}
    \end{equation}
    The final equality is due to the fact that \(\matpdistr\vecone=\vecone\) and \(\matfstar\vecpi=\vecone\)~\cite{BoRaZh11}.
...
Since \(K\) is only related to the structure of graph \(\gr\) itself, equation~\eqref{eq:connection-multiple} indicates a constant difference between GWC \(\manc{S}\) and the group random detour time \(D_{ij}(S)\).
This equation suggests that GWC \(\manc{S}\) can also reflect the extent of detour in the network when \(S\) is set as the "passing group".
This is another way of understanding the physical meaning of GWC.
...
By leveraging this connection along with \thmref{thm:connection-multiple}, we can establish another link between GWC and the equivalent electrical network of the augmented graph.
...
Specifically, we demonstrate that the set function \(\manc{\cdot}\) is both monotone and supermodular, providing us with a theoretical basis for designing greedy algorithms that can provide approximation guarantees for solving the GWC minimization problem.

\begin{theorem}[Monotonicity of GWC]\label{thm:mono}
...
\begin{theorem}[Supermodularity of GWC]\label{thm:supermod}
...
\begin{IEEEproof}
    ...
    So it is obvious that when \(T_{vu}<T_{vT}\) holds true, \(T_{vu}<T_{vS}\) also holds true, thus indicating \(\indfunc{\setof{T_{vu}<T_{vS}}}\ge\indfunc{\setof{T_{vu}<T_{vT}}}\) holds for any vertex \(v\in V\).
...
\begin{problem}[Group Walk Centrality Minimization, GWCM]
...
As a result, we can optimize the simple greedy algorithm outlined in~\algoref{algo:exact-gwcm}, and reduce the time complexity from \(O(kn^4)\) to \(O(kn^3)\).
...
Afterwards, we prove that \algoref{algo:exact-gwcm} has a \(1-\frac{k}{k-1}\cdot\frac{1}{e}\) approximation factor.
\begin{theorem}
...
Despite the time complexity optimization proposed in \algoref{algo:exact-gwcm}, this simple greedy algorithm for solving GWCM still requires matrix inversion, making it unsuitable for large networks.
...
Although~\eqref{eq:denom-solver} suggests that we can avoid matrix inversion by utilizing \lemref{lemma:ST}, the size of the matrices \(\matw'\) and \(\matz\) being \(\abs{E}=m\) and \(\abs{V}=n\) respectively, results in an unacceptable number of calls to the \(\mathtt{Solver}\) function.
...
\subsection{Details of Algorithms \text{ApproxDelta} and \(\text{Approx}\mathcal{GWCM}\)}
...
The \text{ApproxDelta} algorithm calls algorithm \(\mathtt{Solver}\) in \lemref{lem:approx-denom} for \(q=\ceil{24(\epsilon/7)^{-2}\log n}\) times, resulting in a complexity of \(\tilde{O}\mypar{m\epsilon^{-2}}\).
...
\begin{algorithm}
    ...
    \Output{
        The margin \(\Delta(u,S)\) of GWC created by adding vertex \(u\) to \(S\) for any vertex \(u\in V\setminus S\)
    }
    Compute \(\lap\) and \(\vecd\)\;
    ...
    \(\vecx'\gets\mathtt{Solver}(\lap_{-S},\vecd_{-S},\delta_1)\)\;
    ...
    \For{\(i=1,2,\dots,q\)}{
    \(\matx'_{[i,:]}\gets\mathtt{Solver}(\lap_{-S},\overline{\matx}_{[i,:]},\delta_2)\)\;
    \(\maty'_{[i,:]}\gets\mathtt{Solver}(\lap_{-S},\overline{\maty}_{[i,:]},\delta_2)\)\;
    }
    \ForEach{\(u\in V\setminus S\)}{
...
By combining \algoref{ALG01} and \algoref{algo:approxdelta}, we are able to create a faster greedy algorithm called \(\text{Approx}\mathcal{GWCM}\) with a \(1-\frac{k}{k-1}\cdot\frac{1}{e}-\epsilon\) approximation factor.
The complexity of \(\text{Approx}\mathcal{GWCM}\) is \(\tilde{O}\mypar{km\epsilon^{-2}}\), which is nearly linear, as it calls \(\text{Approx\(\mathcal{HK}\)}\) once and \text{ApproxDelta} \(k-1\) times.

\begin{algorithm}
    \caption{\(\text{Approx}\mathcal{GWCM}\)\((\gr,k,\epsilon)\)}
    \label{algo:approx}
    \Input{
        A connected weighted undirected graph \(\gr=(V,E,w)\);
...
We then compare the smallest GWC value with the solution provided by \(\text{Exact}\mathcal{GWCM}\) and \(\text{Approx}\mathcal{GWCM}\), whose results are shown in \figref{pic:compare-effect-optimum}.
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[scale=\scaleoptimum,baseline]
        \begin{axis}[
                ...
                legend entries={\(\text{Approx}\mathcal{GWCM}\),\(\text{Exact}\mathcal{GWCM}\),\(\text{Optimum}\mathcal{GWCM}\)},
            ]
            \addplot+[
                mark=pentagon*,
                mark size = 3pt
    ...
    \begin{tikzpicture}[scale=\scaleoptimum,baseline]
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{GWC \(\manc{S}\) of vertex group \(S\) computed by three different algorithms(\(\text{Exact}\mathcal{GWCM}\),\(\text{Approx}\mathcal{GWCM}\) and \(\text{Optimum}\mathcal{GWCM}\)) on four networks: Zebra (a), Zachary karate club (b), Contiguous USA (c) and Les Miserables (d).\label{pic:compare-effect-optimum}}
\end{figure}

We then compare the performance of \(\text{Exact}\mathcal{GWCM}\) and \(\text{Approx}\mathcal{GWCM}\) with three other algorithms: \text{Top-Absorb}, \text{Top-Degree}, and \text{Top-PageRank}.
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[scale=\scaleexact,baseline]
        \begin{semilogyaxis}[
                ...
                legend entries={\(\text{Approx}\mathcal{GWCM}\),\(\text{Exact}\mathcal{GWCM}\),Top-Absorb,Top-Degree,Top-PageRank},
                ymax=800,
            ]
            \addplot+[
                mark=pentagon*,
    ...
    \begin{tikzpicture}[scale=\scaleexact,baseline]
        ...
        \end{semilogyaxis}
    \end{tikzpicture}
    \caption{GWC \(\manc{S}\) of vertex group \(S\) computed by four different algorithms(\(\text{Exact}\mathcal{GWCM}\),\(\text{Approx}\mathcal{GWCM}\),\text{Top-Absorb} and \text{Top-Degree}) on four networks: CA-GrQc (a), ego-Facebook (b), Euroroads (c) and US power grid (d).\label{pic:compare-effect1}}
\end{figure}
...
\begin{figure}[!t]
    ...
    \begin{tikzpicture}[scale=\scaleexact,baseline]
        ...
        \end{semilogyaxis}
    \end{tikzpicture}
    \caption{GWC \(\manc{S}\) of vertex group \(S\) computed by four different algorithms(\(\text{Exact}\mathcal{GWCM}\),\(\text{Approx}\mathcal{GWCM}\),\text{Top-Absorb} and \text{Top-Degree}) on four networks: Hamster friends (a), Hamster full (b), Reactome (c) and Route views (d).\label{pic:compare-effect2}}
\end{figure}
...
From \tabref{tab:running-time}, we can observe that the running time of \(\text{Approx}\mathcal{GWCM}\) is proportional to the number of edges in the network, leading to an increase in the speedup ratio of \(\text{Approx}\mathcal{GWCM}\) as the network size grows.
...
\begin{table}[htbp]
    ...
    \fontsize{8.0}{8.8}\selectfont
    \begin{threeparttable}
        \caption{The running time (seconds, \(s\)) of \(\text{Exact}\mathcal{GWCM}\) and \(\text{Approx}\mathcal{GWCM}\) (denoted as \text{Exact} and \text{Approx} in the table to save space) with various \(\epsilon\) on several realistic networks}
