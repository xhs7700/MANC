...
Given a graph \(\gr=\mypar{V,E,w}\) with a group of absorbing vertices \(S\), we'll replace all vertices in \(S\) with a new vertex \(s\) to create the augmented graph \(\gr'=\mypar{V',E',w'}\) where \(V'=\mypar{V\setminus S}\cup\setof{s}\).
If we denote the set of neighbours of vertex \(v\) as \(\mathcal{N}_v\), then \(E'\) can be represented as \(E\setminus\setof{\edge{u}{v}|u,v\in S}\cup\setof{\edge{u}{s}|u\in\mathcal{N}_v,v\in S}\), where the original edges keep their weight.
As there may exist multiple edges like \(\edge{u}{s}\), \(w'_{us}\) should be the sum of the weights of all corresponding old edges.
